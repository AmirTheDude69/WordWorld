...
function buildBeginnerBank(lang, seeds, languageLabel) {
  ...
  for (let i = 0; i < BANK_SIZE; i += 1) {
    ...
    const native = seed.native;
    const romanization = seed.romanization;
    const meaning = seed.meaning_en;
    ...
    entries.push({
      ...
      meaning_en: meaning,
      sentence: {
        native: sentenceize(native),
        romanization: sentenceize(romanization),
        meaning_en: meaning,
      },
